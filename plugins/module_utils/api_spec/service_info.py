...
class ServiceInfoSpec:
    ...
    def spec() -> Dict[str, Any]:
        """
        Returns Ansible module spec for service info objects.
        ...
        return dict(
            ...
            gather_subset=dict(
                ...
                default=["config"],
                choices=["all", "config"],
                description="Determines which information to gather about services.",
            ),
            folder=dict(
                type="str",
                required=False,
                description="Filter services by folder container (max 64 chars).",
            ),
            snippet=dict(
                type="str",
                required=False,
                description="Filter services by snippet container (max 64 chars).",
            ),
            device=dict(
                type="str",
                required=False,
                description="Filter services by device container (max 64 chars).",
            ),
            exact_match=dict(
                type="bool",
                required=False,
            ...
            protocol_types=dict(
                ...
                required=False,
                choices=["tcp", "udp"],
                description="Filter by protocol types (tcp, udp).",
            ),
            tags=dict(
                type="list",
                elements="str",
                required=False,
                description="Filter by tags associated with services.",
            ),
            provider=dict(
                type="dict",
                required=True,
