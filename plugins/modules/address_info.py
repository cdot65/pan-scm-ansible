...
"""
Ansible module for gathering information about address objects in SCM.

This module provides functionality to retrieve information about address objects
...
short_description: Gather information about address objects in SCM.

version_added: "0.1.0"

description:
    - Gather information about address objects within Strata Cloud Manager (SCM).
...
options:
    ...
    folder:
        ...
        type: str
    snippet:
        description: Filter addresses by snippet container.
        required: false
        type: str
    device:
        description: Filter addresses by device container.
        required: false
        type: str
    exact_match:
    ...
    exclude_devices:
        ...
        elements: str
    types:
        description: Filter by address types.
        required: false
        type: list
        elements: str
        choices: ["netmask", "range", "wildcard", "fqdn"]
    values:
        description: Filter by address values.
        required: false
        type: list
        elements: str
    tags:
...
EXAMPLES = r"""
---
- name: Gather Address Information in Strata Cloud Manager
  hosts: localhost
  gather_facts: false
  vars_files:
    - vault.yaml
...
RETURN = r"""
addresses:
    description: List of address objects matching the filter criteria (returned when name is not specified).
    ...
    sample:
      ...
      - id: "234e5678-e89b-12d3-a456-426655440001"
        ...
        tag: ["App", "Production"]
address:
    description: Information about the requested address (returned when name is specified).
...
def main():
    """
    Main execution path for the address_info module.

    This module provides functionality to gather information about address objects
    in the SCM (Strata Cloud Manager) system with various filtering options.
    ...
    try:
        ...
        # Check if we're fetching a specific address by name
        if module.params.get("name"):
            ...
            try:
                ...
                address = client.address.fetch(name=name, **container_params)

                # Serialize response for Ansible output
                result["address"] = serialize_response(address)

            except ObjectNotPresentError:
                module.fail_json(
                    msg=f"Address with name '{name}' not found in {list(container_params.keys())[0]} '{list(container_params.values())[0]}'"
                )
            ...
            try:
                addresses = client.address.list(**container_params, **filter_params)

                # Serialize response for Ansible output
                result["addresses"] = [serialize_response(addr) for addr in addresses]
