...
from ansible_collections.cdot65.scm.plugins.module_utils.api_spec.service_connections import ServiceConnectionsSpec
...
def needs_update(existing, params):
    ...
    if hasattr(existing, "qos") and existing.qos is not None:
        current_qos = {
            "enabled": existing.qos.enabled,
            "profile": existing.qos.profile
        }
        update_data["qos"] = current_qos

        if "qos" in params and params["qos"] is not None:
            # Compare qos values and update if different
            if params["qos"].get("enabled") is not None and current_qos["enabled"] != params["qos"]["enabled"]:
                update_data["qos"]["enabled"] = params["qos"]["enabled"]
                changed = True
            if params["qos"].get("profile") is not None and current_qos["profile"] != params["qos"]["profile"]:
    ...
    if hasattr(existing, "backup_connection") and existing.backup_connection is not None:
        ...
        if hasattr(existing.backup_connection, "snippet") and existing.backup_connection.snippet is not None:
            current_backup["snippet"] = existing.backup_connection.snippet
        if hasattr(existing.backup_connection, "device") and existing.backup_connection.device is not None:
        ...
        if "backup_connection" in params and params["backup_connection"] is not None:
            ...
            for key in ["connection_name", "folder", "snippet", "device"]:
                if (key in params["backup_connection"] and 
                    params["backup_connection"][key] is not None and 
                    (key not in current_backup or current_backup[key] != params["backup_connection"][key])):
...
def get_existing_connection(client, connection_data):
    ...
    try:
        ...
        try:
            existing = client.service_connection.fetch(
