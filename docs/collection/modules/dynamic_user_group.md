...
The `dynamic_user_group` module allows you to create, update, and delete dynamic user group objects within Strata Cloud Manager (SCM). Dynamic user groups are used to group users based on tag-based filter expressions, enabling flexible and dynamic security policies based on user attributes.

This module is part of the `cdot65.scm` collection and requires proper authentication credentials to interact with the Strata Cloud Manager API.
